...
static void createProjectTree(Scene *scene, EditorState *editor,
                              ImGuiTreeNodeFlags node_flags,
                              std::string        directory);
void        ExportImGuiStyleSizes();

static void checkPicker(Scene *scene, RenderState *renderer,
                        EditorState *editor) {
    /* if (!editor->isPicking) {
        return;
    } */
    ...
    // glReadPixels(pickPosition.x, renderer->windowData.height - pickPosition.y,
...
static void createEntityTree(Scene *scene, EditorState *editor,
                             uint32_t entityID, ImGuiTreeNodeFlags node_flags,
...
static void createProjectTree(Scene *scene, EditorState *editor,
    ...
    for (const std::filesystem::directory_entry &dir :
        ...
        if (dir.is_directory()) {
            ...
            if (dropped) {
                // Set payload to carry the index of our item (could be anything)
                ImGui::SetDragDropPayload("projectFile", fileString.data(),
                                          fileString.size());
...
static void ShowExampleMenuFile(Scene *scene, Resources *resources,
                                EditorState *editor) {
...
static void buildMainMenu(Scene *scene, Resources *resources,
                          EditorState *editor) {
...
static void buildSceneView(Scene *scene, RenderState *renderer,
                           EditorState *editor, Resources *resources) {
    ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
    ImGui::Begin("EditViewport", nullptr,
    ...
    float aspectRatio = (float)renderer->windowData.viewportWidth /
                        renderer->windowData.viewportHeight;

    ImVec2 imageSize = ImVec2(renderer->windowData.viewportWidth,
                              renderer->windowData.viewportHeight);
    ...
    ImVec2 start = ImVec2((availableSize.x - imageSize.x) / 2,
                          (availableSize.y - imageSize.y) / 2);
    editor->viewportStart = ImVec2(windowPos.x + start.x, windowPos.y + start.y);
    editor->viewportEnd = ImVec2(windowPos.x + start.x + imageSize.x,
                                 windowPos.y + start.y + imageSize.y);

    ImGui::SetCursorPos(start);
    ImGui::Image((ImTextureID)(intptr_t)renderer->editorTex, imageSize,
                 ImVec2(0, 1), ImVec2(1, 0));
    bool droppedPrefab = ImGui::BeginDragDropTarget();

    if (droppedPrefab) {
        if (const ImGuiPayload *payload =
                ImGui::AcceptDragDropPayload("projectFile")) {
            ...
            if (extension == ".prefab") {
                ...
                vec3 pos = getPosition(entities, entities->cameras[0].entityID) +
                           (editor->worldPos * 2.0f);
                setPosition(entities, id, pos);

            } else if (extension == ".gltf") {
                Model *prefab = resources->modelMap[fileDragged];
                id = createEntityFromModel(&scene->entities, &scene->physicsScene,
                                           prefab->rootNode, INVALID_ID, false,
...
static void buildProjectFiles(Scene *scene, Resources *resources,
                              EditorState *editor) {
    ImGui::Begin("Project");
    bool droppedPrefab = ImGui::BeginDragDropTarget();
...
static void buildEnvironmentSettings(RenderState *renderer, EditorState *editor,
                                     Scene *scene) {
    ImGui::Begin("Post-Process");
    ImGui::DragFloat("Exposure", &renderer->exposure, 0.01f, 0, 1000.0f);
    ImGui::DragFloat("Bloom Threshold", &renderer->bloomThreshold, 0.01f, 0,
                     100.0f);
    ...
    ImGui::Checkbox("Crazy Town", &renderer->crazyTown);
    if (ImGui::Checkbox("Resize", &renderer->resize)) {
        if (renderer->resize) {
            renderer->windowData.viewportWidth = 800;
            renderer->windowData.viewportHeight = 600;
        } else {
    ...
    vec2  cursorPos(scene->input->cursorPosition.x,
                    scene->input->cursorPosition.y);
    ...
    if (cursorPos.x > editor->viewportStart.x &&
        cursorPos.x < editor->viewportEnd.x &&
        cursorPos.y > editor->viewportStart.y &&
        cursorPos.y < editor->viewportEnd.y) {
...
void updateAndDrawEditor(Scene *scene, RenderState *renderer,
                         Resources *resources, EditorState *editor) {
...
static void cameraControlUpdate(Scene *scene, Resources *resources,
                                RenderState *renderer, EditorState *editor) {
    EntityGroup  *entities = &scene->entities;
    ...
    vec3 cameraTargetRotation =
        vec3(JPH::DegreesToRadians(editor->cameraController.pitch),
             JPH::DegreesToRadians(editor->cameraController.yaw), 0.0f);

    setRotation(entities, transform->entityID,
                quat::sEulerAngles(cameraTargetRotation));
    ...
    vec3 finalMove =
        moveDir * editor->cameraController.moveSpeed * scene->deltaTime;
...
static void defaultUpdate(Scene *scene, Resources *resources,
                          RenderState *renderer, EditorState *editor) {
...
static void pickingUpdate(Scene *scene, EditorState *editor,
...
void updateEditor(Scene *scene, Resources *resources, RenderState *renderer,
                  EditorState *editor) {
...
void initEditor(EditorState *editor, GLFWwindow *window) {
    ...
    io.Fonts->AddFontFromFileTTF("../resources/fonts/Karla-Regular.ttf",
                                 aspectRatio * 8);
