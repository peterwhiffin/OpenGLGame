...
void drawPickingScene(RenderState *renderer, EntityGroup *entities) {
    ...
    for (int i = 0; i < entities->meshRenderers.size(); i++) {
        ...
        glUniformMatrix4fv(uniform_location::kModelMatrix, 1, GL_FALSE,
                           &model(0, 0));
...
static void drawScene(RenderState *renderer, EntityGroup *entities,
    ...
    for (int i = 0; i < entities->meshRenderers.size(); i++) {
        ...
        if (renderer->crazyTown) {
            float dist = vec3Distance(getPosition(entities, camera->entityID),
                                      getPosition(entities, meshRenderer->entityID));
            float fov = remap(dist, 5, 100, 110, 8);
            camera->fov = fov;
        ...
        if (meshRenderer->boneMatrices.size() > 0) {
            ...
            glUniformMatrix4fv(glGetUniformLocation(renderer->lightingShader,
                                                    "finalBoneMatrices[0]"),
                               meshRenderer->boneMatrices.size(), GL_FALSE,
                               &meshRenderer->boneMatrices[0](0, 0));
        }
...
#ifdef PETES_EDITOR
            // vec4 baseColor = scene->pickedEntity == meshRenderer->entityID ?
            // vec4(0.0f, 1.0f, 0.0f, 1.0f) : material->baseColor;
...
#endif
            glUniform2fv(
                glGetUniformLocation(renderer->lightingShader, "textureTiling"), 1,
                glm::value_ptr(material->textureTiling));
...
void MyDebugRenderer::DrawLine(JPH::RVec3Arg inFrom, JPH::RVec3Arg inTo,
                               JPH::ColorArg inColor) {
    lines.push_back({inFrom, inTo, inColor});
}

void MyDebugRenderer::DrawTriangle(JPH::RVec3Arg inV1, const JPH::RVec3Arg inV2,
...
void MyDebugRenderer::DrawText3D(JPH::RVec3Arg           inPosition,
...
void renderDebug(RenderState *renderer) {
    ...
    MyDebugRenderer *debug =
        static_cast<MyDebugRenderer *>(renderer->debugRenderer);
    ...
    glBufferData(GL_ARRAY_BUFFER, lineVerts.size() * sizeof(DebugVertex),
    ...
    glBufferData(GL_ARRAY_BUFFER, triVerts.size() * sizeof(DebugVertex),
...
static void createPickingFBO(RenderState *renderer) {
    ...
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                              GL_RENDERBUFFER, renderer->pickingRBO);
...
void createSpotLightShadowMap(SpotLight *light) {
    ...
    glBindTexture(GL_TEXTURE_2D, light->depthTex);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_DEPTH_COMPONENT32F, light->shadowWidth,
                 light->shadowHeight, 0, GL_DEPTH_COMPONENT, GL_FLOAT, NULL);
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        std::cerr << "ERROR::FRAMEBUFFER:: Depth framebuffer is not complete!"
                  << std::endl;
    }
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, light->shadowWidth,
                 light->shadowHeight, 0, GL_RED, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_BORDER);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
...
void deleteSpotLightShadowMap(SpotLight *light) {
    // this may break if the textures/framebuffers haven't been created in the
    // case of a prefab. we'll see.
    GLuint textures[2] = {light->depthTex, light->blurDepthTex};
    GLuint frameBuffers[2] = {light->depthFrameBuffer,
                              light->blurDepthFrameBuffer};
    glDeleteTextures(2, textures);
    glDeleteFramebuffers(2, frameBuffers);
}

static void createShadowMapDepthBuffers(
    EntityGroup *entities) {  // this needs to just be callable per light, not
                              // looping over every light.
...
static void createEditorBuffer(RenderState *renderer) {
    ...
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        std::cerr << "ERROR::FRAMEBUFFER::Editor framebuffer is not complete!"
                  << std::endl;
    }
...
static void createSSAOBuffer(RenderState *renderer) {
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, renderer->ssaoFBO);
    glBindTexture(GL_TEXTURE_2D, renderer->blurTex);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16F, renderer->windowData.viewportWidth,
                 renderer->windowData.viewportHeight, 0, GL_RGBA, GL_FLOAT, NULL);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                           renderer->blurTex, 0);
    if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
        std::cerr << "ERROR::FRAMEBUFFER:: SSAO Color framebuffer is not complete!"
                  << std::endl;
    }
}
...
static void resizeBuffers(RenderState *renderer) {
    ...
    glBindFramebuffer(GL_FRAMEBUFFER, renderer->ssaoFBO);
    glBindTexture(GL_TEXTURE_2D, renderer->blurTex);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA16, renderer->windowData.viewportWidth,
                 renderer->windowData.viewportHeight, 0, GL_RGBA, GL_FLOAT, NULL);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                           renderer->blurTex, 0);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, NULL);
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D,
                           renderer->editorTex, 0);

    glBindRenderbuffer(GL_RENDERBUFFER, renderer->editorRBO);
    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, width, height);
    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                              GL_RENDERBUFFER, renderer->editorRBO);
...
static void createFullScreenQuad(RenderState *renderer) {
    ...
    glVertexAttribPointer(vertex_attribute_location::kVertexPosition, 3, GL_FLOAT,
                          GL_FALSE, 5 * sizeof(float), (void *)0);

    glEnableVertexAttribArray(vertex_attribute_location::kVertexTexCoord);
    glVertexAttribPointer(vertex_attribute_location::kVertexTexCoord, 2, GL_FLOAT,
...
static void generateSSAOKernel(RenderState *renderer) {
    ...
    for (unsigned int i = 0; i < 8; i++) {
        glUniform3fv(
            glGetUniformLocation(renderer->ssaoShader,
                                 ("samples[" + std::to_string(i) + "]").c_str()),
            1, renderer->ssaoKernel[i].mF32);
    }
}

void deleteBuffers(RenderState *renderer, Resources *resources) {
    unsigned int textures[5] = {renderer->pickingTex, renderer->litColorTex,
...
void initializeEnvironment(RenderState *renderer, Scene *scene,
                           unsigned int shader) {
    float test[2];
...
void updateBufferData(RenderState *renderer, Scene *scene) {
    ...
    renderer->matricesUBOData.view = mat4::sLookAt(
        position, position + transformForward(entities, camera->entityID),
        transformUp(entities, camera->entityID));
    if (camera->isPerspective) {
        renderer->matricesUBOData.projection =
            mat4::sPerspective(camera->fovRadians, renderer->windowData.aspectRatio,
                               camera->nearPlane, camera->farPlane);
    } else {
        float     orthoSize = camera->fov;
        glm::mat4 proj = glm::ortho(-orthoSize, orthoSize, -orthoSize, orthoSize,
                                    camera->nearPlane, camera->farPlane);
    ...
    glBufferSubData(GL_UNIFORM_BUFFER, 0, sizeof(GlobalUBO),
                    &renderer->matricesUBOData);
}

void renderScene(RenderState *renderer, EntityGroup *entities, Scene *scene) {
    drawShadowMaps(renderer, entities);
    drawScene(renderer, entities, scene);
    drawSSAO(renderer);
...
void onScreenChanged(GLFWwindow *window, int width, int height) {
    ...
    glUseProgram(renderer->ssaoShader);
    vec2 v(renderer->windowData.viewportWidth / 4.0f,
           renderer->windowData.viewportHeight / 4.0f);
...
void createContext(RenderState *renderer) {
    ...
    renderer->window =
        glfwCreateWindow(renderer->windowData.width, renderer->windowData.height,
                         "Pete's Game", NULL, NULL);
